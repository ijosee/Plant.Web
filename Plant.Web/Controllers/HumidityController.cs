...
namespace Plant.Web.Controllers {
    public class HumidityController : Controller {
        ...
        [HttpGet]
        public async Task<DataTableRs<HumidityLogRs>> GetDataTable (DataTableRq request) {
            var result = new DataTableRs<HumidityLogRs> ();
            try {
                _logger.LogDebug ("Getting sensor data from api");
                var baseUrl = _configuration.GetSection ("PlantApi").GetSection ("BaseUrl").Value.ToString ();
                _logger.LogInformation ($"ApiBaseUrl -> {baseUrl}");
                var httpRequest = new HttpRequestMessage (HttpMethod.Post,
                    $"{baseUrl}api/Humidity/GetDataTable");
                ...
                if (response.IsSuccessStatusCode) {
                    result = await response.Content.ReadAsAsync<DataTableRs<HumidityLogRs>> ();
                } else {
                    result = null;
                }
